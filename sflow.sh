...
start() {
    ...
    docker create -t --cap-add=NET_ADMIN --cap-add=SYS_ADMIN -v /etc/sonic:/etc/sonic:ro -v /etc/localtime:/etc/localtime:ro  -v /host/warmboot:/var/warmboot \
        --net=$NET \
