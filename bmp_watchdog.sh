...
start() {
    ...
    docker create -t --privileged --pid=host -v /lib/systemd/system:/lib/systemd/system:rw -v /etc/sonic:/etc/sonic:ro -v /etc/localtime:/etc/localtime:ro \
        --net=$NET \
	...
	--tmpfs /var/log/supervisor:rw \
        ...
        --name=$DOCKERNAME \
        docker-bmp-watchdog-dbg:latest \
        || {
