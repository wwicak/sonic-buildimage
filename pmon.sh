...
start() {
    ...
    docker create --privileged -t -v /etc/sonic:/etc/sonic:ro -v /etc/localtime:/etc/localtime:ro  -v /host/reboot-cause:/host/reboot-cause:rw -v /host/pmon/stormond:/usr/share/stormond:rw -v /var/run/platform_cache:/var/run/platform_cache:ro -v /usr/share/sonic/device/pddf:/usr/share/sonic/device/pddf:ro \
        --net=$NET \
    ...
    -v /usr/share/sonic/firmware:/usr/share/sonic/firmware:rw \
        ...
        --name=$DOCKERNAME \
        docker-platform-monitor-dbg:latest \
        || {
