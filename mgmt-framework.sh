...
start() {
    ...
    docker create -t -v /etc/sonic:/etc/sonic:ro -v /etc/localtime:/etc/localtime:ro  -v /etc:/host_etc:ro -v /var/run/dbus:/var/run/dbus:rw --mount type=bind,source=/var/platform/,target=/mnt/platform/ \
        --net=$NET \
	...
	--tmpfs /var/log/supervisor:rw \
        ...
        --name=$DOCKERNAME \
        docker-sonic-mgmt-framework-dbg:latest \
        || {
